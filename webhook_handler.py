...
class FeishuWebhook:
    ...
    def send_data(self, data, pdf_filename):
        ...
        try:
            ...
            for book in data:
                ...
                # 直接使用Gemini提供的中文内容，不再进行翻译
                # Gemini已经以中文输出内容，包括适当保留的英文原文
                title = title_value
                author = author_value
    ...
    def save_translated_data(self, data, pdf_filename, use_chinese_fields=False, already_in_chinese=True):
        """保存结构化的书籍数据到本地JSON文件
        
        Args:
            data: 需要保存的书籍数据
            pdf_filename: PDF文件名
            use_chinese_fields: 如果为True，使用中文字段名；否则使用原始英文字段名
            already_in_chinese: 已废弃参数，保留仅为兼容性
        """
        if not data:
            logger.warning(f"No data to save for {pdf_filename}")
            return False
        ...
        try:
            ...
            if os.path.exists(translated_cn_fields_file):
                logger.info(f"翻译文件已存在，跳过重复处理：{translated_cn_fields_file}")
                return True
                
            # 准备保存数据
            logger.info(f"Saving data for {len(data)} book entries from {pdf_filename}")
            ...
            print(f"开始处理 {total_books} 本书的信息...")
            
            for i, book in enumerate(data):
                # 显示进度
                progress = int((i+1) / total_books * 100)
                print(f"\r处理进度: [{progress}%] {'=' * (progress//5)}{' ' * (20-progress//5)}", end="")
                
                # 提取书籍信息 - 直接使用Gemini提供的中文内容
                title = book.get("title", book.get("标题", book.get("主题", "")))
                author = book.get("Author", book.get("作者", ""))
                summary = book.get("Summary", book.get("摘要", ""))
                publisher = book.get("Publisher", book.get("出版社", ""))
                date = book.get("Date", book.get("出版日期", book.get("日期", "")))
                theme = book.get("Theme", book.get("主题", ""))
                author_bio = book.get("Author_bio", book.get("作者简介", ""))
                ...
                translated_book = {
                    ...
                    "Theme": theme,
                    "Author_bio": author_bio,
                    "original_title": title  # 现在title本身就包含了中英文标题
                }
